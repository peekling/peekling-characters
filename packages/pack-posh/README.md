...
```sh
npm install @peekling/pack-posh@0.1.0
```
...
Posh is licensed under Apache-2.0. Redistributions must comply with Section 4 of the Apache License 2.0, including its applicable notice-retention requirements. See this package's `LICENSE` and `NOTICE` files.

Run `npm test -w @peekling/pack-posh` to validate the pack, or `npm run build -w @peekling/pack-posh` to rebuild and run artwork QA.
