...
const artifactsRoot = path.resolve(sourceRoot, "../../../artifacts/art/nib");
...
if (
  ...
  throw new Error("Nib generated output must stay under artifacts/art/nib");
...
export async function buildPack() {
  const sourceBytes = await readFile(
    path.join(sourceRoot, "nib-racing-sheet-v2.png"),
  );
  const sheet = PNG.sync.read(sourceBytes);
  if (sheet.width !== 1536 || sheet.height !== 1024)
    throw new Error("Nib source pose sheet must be 1536x1024");
  ...
  if (report.uniqueRenderedFrames1x !== FRAME_COUNT)
    throw new Error("Nib contains repeated rendered signature frames at 1x");
  ...
  await writeFile(
    path.join(outputParent, "nib-qa.json"),
    `${JSON.stringify(report, null, 2)}\n`,
  );
  await writeFile(
    path.join(outputParent, "nib-state-sheet.png"),
    encode(createStateSheet(oneX, frameBlueprints)),
  );
  await writeFile(
    path.join(outputParent, "nib-state-map.md"),
...
function extractPoses(sheet) {
  ...
  if (components.length !== 24)
    throw new Error(
      `Nib source must contain 24 connected character poses, found ${components.length}`,
    );
...
function normalizePose(input) {
  ...
  if (!bounds) throw new Error("Nib source pose is empty");
...
function renderFrame(frame, poses) {
  ...
  if (frame.group === "motion") drawRollingWheels(output, frame);
...
function drawEffect(image, frame, behind) {
  ...
  if (
    behind &&
    ["shield", "hologram", "sync", "charge", "pit-stop"].includes(effect)
  ) {
  ...
  switch (effect) {
    ...
    case "download":
      ...
      break;
    case "pit-stop":
      drawRing(image, 101, 32, 9, 2, GOLD);
      drawLine(image, 94, 39, 108, 25, 2.4, GOLD);
      drawCircle(image, 101, 32, 2.5, CYAN);
      break;
    case "scan":
    ...
    case "stealth":
      ...
      break;
    case "drift":
      drawArc(image, 64, 108, 28, Math.PI * 0.08, Math.PI * 0.92, 2.4, CYAN);
      drawLine(image, 30, 112, 12, 118, 2, PALE_CYAN);
      break;
    case "overclock":
      ...
      break;
    default:
      throw new Error(`Unknown Nib effect: ${effect}`);
  }
}

function drawMotionTrail(image, direction, gait, variant) {
  const [dx, dy] = direction;
  const color = gait === "boost" ? GOLD : gait === "race" ? CYAN : PURPLE;
  const length = gait === "boost" ? 44 : gait === "race" ? 32 : 20;
...
function drawRollingWheels(image, frame) {
  const phase = frame.wheelPhase * (Math.PI / 8);
  const wheelCenters = [
    [38, 99],
    [91, 99],
  ];
  for (const [centerX, centerY] of wheelCenters) {
    drawRing(image, centerX, centerY, 10, 1.5, [25, 31, 40, 235]);
    for (let spoke = 0; spoke < 4; spoke++) {
      const angle = phase + spoke * (Math.PI / 2);
      drawLine(
        image,
        centerX + Math.cos(angle) * 3,
    ...
    drawCircle(image, centerX, centerY, 2.5, CYAN);
  }
}
...
function createStateMap(frames) {
  const lines = [
    "# Nib state map",
    "",
    "Every state starts on its own signature frame. Nib keeps the same racing body, windshield eyes, gold stripe, spoiler, and wheels while posture, wheel phase, and telemetry effects communicate the performance.",
    "",
...
if (
  ...
  console.log(
    `Built ${FRAME_COUNT} Nib states at 1x, 2x, and 4x in ${result.outputRoot}`,
  );
}
