...
X3 is licensed under Apache-2.0. Redistributions must comply with Section 4 of the Apache License 2.0, including its applicable notice-retention requirements. See this package's `LICENSE` and `NOTICE` files.

Run `npm test -w @peekling/pack-x3` to validate the pack, or `npm run build -w @peekling/pack-x3` to rebuild and run artwork QA.
